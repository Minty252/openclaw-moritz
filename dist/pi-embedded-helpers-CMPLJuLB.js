...
function clampToBudget(content, budget) {
	...
	return `${truncateUtf16Safe(content, Math.max(1, budget - 1))}…`;
}
...
async function loadPwAiModule(mode) {
	try {
		return await import("./pw-ai-CVoJzobE.js");
	} catch (err) {
...
function createProfileContext(opts, profile) {
	...
	const resetProfile = async () => {
		...
		if (await isHttpReachable(300) && !profileState.running) try {
			await (await import("./pw-ai-CVoJzobE.js")).closePlaywrightBrowserConnection();
		} catch {}
...
async function startBrowserBridgeServer(params) {
	...
	if (authToken || authPassword) app.use((req, res, next) => {
		if (isAuthorizedBrowserRequest(req, {
			token: authToken,
...
//#endregion
export { updateSessionStore as $, expandPolicyWithPluginGroups as $t, isCompactionFailureError as A, resolveBrowserConfig as At, parseImageSizeError as B, resolveSkillsPromptForRun as Bt, formatAssistantErrorText as C, resolveChannelGroupPolicy as Ct, isAuthAssistantError as D, resolveGroupSessionKey as Dt, getApiErrorPayloadFingerprint as E, resolveConversationLabel as Et, isRateLimitAssistantError as F, resolvePathsWithinRoot as Ft, extractDeliveryInfo as G, applySkillEnvOverrides as Gt, ensureSandboxWorkspaceForSession as H, assertMediaNotDataUrl as Ht, isRawApiErrorPayload as I, getBridgeAuthForPort as It, loadSessionStore as J, resolveBrowserControlAuth as Jt, appendAssistantMessageToSessionTranscript as K, applySkillEnvOverridesFromSnapshot as Kt, isTimeoutErrorMessage as L, buildWorkspaceSkillCommandSpecs as Lt, isFailoverAssistantError as M, getMediaDir as Mt, isFailoverErrorMessage as N, saveMediaBuffer as Nt, isBillingAssistantError as O, createBrowserRouteContext as Ot, isLikelyContextOverflowError as P, DEFAULT_UPLOAD_DIR as Pt, updateLastRoute as Q, collectExplicitAllowlist as Qt, isTransientHttpError as R, buildWorkspaceSkillSnapshot as Rt, classifyFailoverReason as S, resolveIMessageAccount as St, formatRawAssistantErrorForUi as T, resolveChannelGroupToolsPolicy as Tt, resolveSandboxContext as U, assertSandboxPath as Ut, sanitizeUserFacingText as V, resolvePluginSkillDirs as Vt, resolveSandboxRuntimeStatus as W, resolveSandboxedMediaSource as Wt, recordSessionMetaFromInbound as X, applyOwnerOnlyToolPolicy as Xt, readSessionUpdatedAt as Y, resolveSandboxConfigForAgent as Yt, saveSessionStore as Z, buildPluginToolGroups as Zt, sanitizeSessionMessagesImages as _, canonicalizeMainSessionAlias as _t, formatXHighModelHint as a, matchesAnyGlobPattern as an, mergeDeliveryContext as at, isGoogleModelApi as b, getChannelDock as bt, normalizeReasoningLevel as c, resolveBootstrapMaxChars as cn, acquireSessionWriteLock as ct, normalizeVerboseLevel as d, listEnabledSignalAccounts as dn, resolveChannelResetConfig as dt, expandToolGroups as en, updateSessionStoreEntry as et, resolveResponseUsageMode as f, resolveSignalAccount as fn, resolveSessionResetPolicy as ft, normalizeTextForComparison as g, resolveFreshSessionTotalTokens as gt, isMessagingToolDuplicateNormalized as h, DEFAULT_RESET_TRIGGERS as ht, formatThinkingLevels as i, compileGlobPatterns as in, deliveryContextKey as it, isContextOverflowError as j, resolveProfile as jt, isCloudCodeAssistFormatError as k, registerBrowserRoutes as kt, normalizeThinkLevel as l, resolveBootstrapTotalMaxChars as ln, resolveSessionKey as lt, isMessagingToolDuplicate as m, resolveThreadFlag as mt, validateGeminiTurns as n, resolveToolProfilePolicy as nn, resolveCacheTtlMs as nt, listThinkingLevels as o, buildBootstrapContextFiles as on, normalizeDeliveryContext as ot, supportsXHighThinking as p, resolveSessionResetType as pt, resolveMirroredTranscriptText as q, ensureBrowserControlAuth as qt, pickFallbackThinkingLevel as r, stripPluginOnlyAllowlist as rn, deliveryContextFromSession as rt, normalizeElevatedLevel as s, ensureSessionHeader$1 as sn, normalizeSessionDeliveryFields as st, validateAnthropicTurns as t, normalizeToolName as tn, isCacheEnabled as tt, normalizeUsageDisplay as u, sanitizeGoogleTurnOrdering as un, evaluateSessionFreshness as ut, downgradeOpenAIReasoningBlocks as v, resolveMainSessionKey as vt, formatBillingErrorMessage as w, resolveChannelGroupRequireMention as wt, BILLING_ERROR_USER_MESSAGE as x, listChannelDocks as xt, isAntigravityClaude as y, deriveSessionMetaPatch as yt, parseImageDimensionError as z, loadWorkspaceSkillEntries as zt };